...
const deployApp = async () => {
  ...
  const approvalProgram = await getBasicProgramBytes(
    "./artifacts/sc_approval.teal"
  );
  const clearProgram = await getBasicProgramBytes(
    "./artifacts/sc_clearstate.teal"
  );
  ...
  console.log("PLEASE SAVE THIS IN THE ENV FILE");
...
const transferAlgos = async (to, amount) => {
  console.log(
    `Transferring ${amount} microalgos from ${creator.addr} to ${to}`
  );
  const suggestedParams = await algodClient.getTransactionParams().do();
...
const topupContract = async (appId, boxName, boxSize) => {
  ...
  console.log(
    `Box requires ${minBalance} microAlgos, ${minBalance / 1e6} Algos`
  );
...
const getApp = async () => {
  ...
  return Number(process.env.APP_ID);
};
